...
### 🔌 EV charging — steady offer (hold the box, then track like evcc)
- 🐛 **The offered current flapped 6↔9 A every few seconds**, so a steady-needing car (Renault Zoe) sat in standby. Root cause: the KEBA reverted to its built-in **6 A failsafe** between SEM's writes and SEM re-wrote 9 A every 5 s — a sawtooth the car can't charge through. Cross-checked against **evcc**, which doesn't flap because it never fights a failsafe. Fix is two parts: **(1)** SEM now *persists* its benign failsafe (`failsafe_persist=1`) so it **overwrites** the box's 6 A with a fallback at your charging floor — the box holds the offer instead of reverting; **(2)** the current then tracks surplus evcc-style (≈30 s cadence with a 2 A deadband, not a multi-minute freeze), with evcc-aligned **1 min start / 3 min stop** delays (disable delay 300→180 s). The 5 s heartbeat is kept to feed the failsafe watchdog. (#546)
- ⚙️ Hidden escape hatch: `keba_steady_failsafe` (default **on**) reverts to the legacy non-persistent failsafe if needed. **Footprint note:** with it on, SEM writes a persistent failsafe into the KEBA's stored config (re-armed with your current floor at each session start); if you uninstall SEM mid-session the box keeps that fallback until reset. (#546)
- 🔭 The `EV-OFFER-PROBE` diagnostic now reads the **live** offered-current sensor (`ev_current_sensor`, e.g. `sensor.keba_p30_max_current`) instead of the static config cap, so it can actually show hardware drift. (#546)
