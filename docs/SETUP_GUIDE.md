...
- **Dynamic**: SEM reads the current rate from a HA sensor every cycle. Used
  ...
  change hourly. The [overnight joint planner](OVERNIGHT_PLANNER.md) uses the
...
[overnight joint planner](OVERNIGHT_PLANNER.md), which decides *when*: the
...
> `switch.sem_overnight_actuation` **off** there is no planned pre-charge at
