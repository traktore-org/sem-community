...
- **Daily runtime goals for household loads (#620)** — give any switch a **Min / Max runtime** window (e.g. *pool pump ≥ 4 h, ≤ 8 h/day*) via a dual-handle slider, a **mode** (Off / Peak-only / Solar only / **Solar + battery**), and an optional stop condition picked with an entity search (e.g. water-temp ≥ 28 °C, car SOC ≥ 80 %). In *Solar + battery* mode the home battery **assists the load above the buffer** (daytime) and can **finish the remaining runtime overnight** down to the reserve floor — no forced grid deadlines. The load is prioritised in the one device list and exposes a surplus-event interface for your own automations. See [`docs/LOAD_PRIORITY.md`](docs/LOAD_PRIORITY.md).
