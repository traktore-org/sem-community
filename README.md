...
- **Daily runtime targets for household loads** — give any switch a *minimum runtime* goal (e.g. *pool pump ≥ 4 h/day*): it runs on solar surplus first, prioritised in the one device list, with an optional stop condition (e.g. car SOC ≥ 80 % on a dumb socket) and a surplus-event interface for your own automations. Guaranteed-by-deadline grid/battery top-up and a max-runtime cap are tracked in [#620](https://github.com/traktore-org/sem-community/issues/620).
