"""#523 (RienduPre, Nibe SG-Ready) — relay map matches the standard + NC invert.
...
("the heat pump never got turned on"). This pins the corrected standard map and
the per-pump ``invert_sg_ready`` toggle for normally-closed contact wiring.
"""
...
class TestStandardTruthTable:
    ...
    async def test_boost_does_not_block_the_pump(self):
        ...
        calls = {c.args[1]: c.args[2]["entity_id"]
