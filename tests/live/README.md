...
- **`test_night_peak_rolling.sh`** — pins the #288 rolling-15min-based throttle. SKIPs cleanly when not in NIGHT_CHARGING_ACTIVE or when the rolling is below the peak limit. Catches a regression that flips the formula back to `power.home_consumption_power`.
